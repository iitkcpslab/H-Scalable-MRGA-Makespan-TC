...
'''
        Trigger module - for executing OM+OTC + path planning (tswap's)  
'''
...
def execute_set( set_param ):
	...
	while ( loop_no <= total_loops ):
		...
		print("Time taken for HEUR case OM: ", heur_t_om, "seconds", file=logf)
		print("Time taken for HEUR case OTC: ", heur_t_otc, "seconds", file=logf)
		print("No of robot-goal paths explored in HEUR case JUST FOR OPTIMAL MAKESPAN: ", nexp_makespan_h, file=logf)
		print("No of robot-goal paths explored in HEUR case for optimal MAKESPAN + Hungarian: ", no_of_exp_h, file=logf)
		...
		print("COL-FREE-TIME for HEUR case: ", col_free_time_h, file=logf)
		print("COL-FREE-COST for HEUR case: ", col_free_cost_h, file=logf)

		print("CL count for HEUR case OM only: ", makespan_CL, file=logf)
		print("CL count for HEUR case OM+OTC only: ", heur_CL, file=logf)
		...
		print("\n\nOUR ALGO:--", file=logf)
		print("\nTime taken for HEUR case (seconds):", total_Htime, file=logf)
		print("Time taken for HEUR case OM: ", heur_t_om, "seconds", file=logf)
		print("Time taken for HEUR case OTC: ", heur_t_otc, "seconds", file=logf)
	...
	return return_time_h, return_exp_h
